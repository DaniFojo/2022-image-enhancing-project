...
class DecomNet(nn.Module):
	...
	def __init__(self, out_channels=64, kernel_size=3):
		...
		self.conv0 = nn.Conv2d(in_channels=3, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')  

		# 5 convolutional layers with a ReLU 
		self.conv1 = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')
		self.conv2 = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')
		self.conv3 = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')
		self.conv4 = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')
		self.conv5 = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')

		# Convolutional with no activation function:
		self.conv6 = nn.Conv2d(in_channels=out_channels, out_channels=4, kernel_size=kernel_size, stride=1, padding='same')
	...
	def forward(self, x):
		...
		I = F.sigmoid(x[:,3:4,:,:]) # n x c x h x w
		return R,I
...
class RelightNet(nn.Module):
	...
	def __init__(self, out_channels=64, kernel_size=3):
		...
		self.conv3 = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=2, padding=self.padding)

		# Decoder
		self.deconv1 = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')
		self.deconv2 = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')
		self.deconv3 = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')

		# self.deconv1 = nn.ConvTranspose2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')
		# self.deconv2 = nn.ConvTranspose2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')
		# self.deconv3 = nn.ConvTranspose2d(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, stride=1, padding='same')
...
class ImageEnhance(nn.Module):
	...
	def __init__(self, out_channels=64, kernel_size=3):
		super().__init__()
		self.decom = DecomNet(out_channels, kernel_size)
		self.relight = RelightNet(out_channels, kernel_size)

	def forward(self, x):
		R, I = self.decom(x)
		x = self.relight(torch.concat((R,I), dim=1))
		return x
...
def gradient(input_tensor, direction):
	smooth_kernel_x = torch.reshape(torch.tensor([[0, 0], [-1, 1]], dtype=torch.float32), (1, 1, 2, 2))
...
def loss_decomNet(input_low, input_high, r_low, i_low, r_high, i_high):
	...
	loss_decom = loss_recon_low + loss_recon_high + 0.001 * loss_recon_mutal_low + 0.001 * loss_recon_mutal_high + 0.01 * loss_invariable_reflectance + 0.1 * loss_illumination_smoothness_low + 0.1 * loss_illumination_smoothness_high
	return loss_decom
