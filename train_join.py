...
def forward_losses(model_decom, model_rel, img_low, img_high, ignore_ienhanace=False):
...
def train(model_decom, model_relight, train_loader, opt, epoch, ignore_ienhanace=False):
    losses_decom = []
    losses_relight = []
    losses_total = []
    model_decom.train()
    ...
    for j, (img_low, img_high) in enumerate(train_loader):
        opt.zero_grad()
        i_low, i_high, r_low, r_high, i_enhanced, reconstructed, loss_decom, loss_relight, loss = forward_losses(model_decom, model_relight, img_low, img_high, ignore_ienhanace)
    ...
    logger.log_images_grid(img_low=img_low, img_high=img_high, i_low=i_low, i_high=i_high, r_high=r_high, r_low=r_low, i_enhanced=i_enhanced, reconstructed=reconstructed, mode='tr', net='join', step=epoch)
...
def eval(model_decom, model_relight, val_loader, epoch, ignore_ienhanace=False):   
    losses_decom = []
    losses_relight = []
    losses_total = []
    with torch.no_grad():
    ...
    loss_mean_decomposition = np.mean(losses_decom)
    loss_mean_relight = np.mean(losses_relight)
    logger.log_images_grid(img_low=img_low, img_high=img_high, i_low=i_low, i_high=i_high, r_high=r_high, r_low=r_low, i_enhanced=i_enhanced, reconstructed=reconstructed, mode='vl', net='join', step=epoch)
...
def training_join(n_epochs, decom_lr, relight_lr, s_epochs, ignore_ienhanace=False):
    ...
    for epoch in range(1, n_epochs+1):
        ...
        print("Training")
        train(model_decomposition, model_relight, train_data_loader, optimizer, epoch, ignore_ienhanace)

        print("Validation")
        loss_mean_total = eval(model_decomposition, model_relight, val_data_loader, epoch, ignore_ienhanace)
