...
def train_decom(model_decom, train_loader, opt, epoch):
    ...
    logger.log_images_grid(img_low=img_low, img_high=img_high, i_low=i_low, i_high=i_high, r_low=r_low, r_high=r_high, mode='tr', net='decom', step=epoch)
    logger.log_loss(loss=loss_mean_decomposition, mode='tr', net='decom', step=epoch)
...
def train_relight(model_decom, model_rel, train_loader, opt, epoch):
    ...
    logger.log_images_grid(img_low=img_low, img_high=img_high, i_low=i_low, r_low=r_low, i_enhanced=i_enhanced, reconstructed=reconstructed, mode='tr', net='rel', step=epoch)
...
def eval_decom(model_decom, val_loader, epoch):
    ...
    with torch.no_grad():
        ...
        logger.log_images_grid(img_low=img_low, img_high=img_high, i_low=i_low, i_high=i_high, r_low=r_low, r_high=r_high, mode='vl', net='decom', step=epoch)
        logger.log_loss(loss=loss_mean_decomposition, mode='vl', net='decom', step=epoch)
...
def eval_relight(model_decom, model_rel, val_loader, epoch):
    ...
    with torch.no_grad():
        ...
        logger.log_images_grid(img_low=img_low, img_high=img_high, i_low=i_low, r_low=r_low, i_enhanced=i_enhanced, reconstructed=reconstructed, mode='vl', net='rel', step=epoch)
...
def training_split(n_epochs, decom_lr, relight_lr, s_epochs):
    ...
    path_decomposition = os.path.join('checkpoints', 'split', 'decomposition', date_dir)
    ...
    for epoch in range(1, n_epochs+1):
        ...
        train_decom(model_decomposition, train_data_loader, optimizer_decomposition, epoch)

        print("Validation Decompostion")
        loss_mean_decomposition = eval_decom(model_decomposition, val_data_loader, epoch)
        scheduler_decomposition.step(loss_mean_decomposition)
        logger.log_learning_rate(optimizer_decomposition, net='decom', step=epoch)
    ...
    for epoch in range(n_epochs+1, 2*n_epochs+1):
        ...
        train_relight(model_decomposition, model_relight, train_data_loader, optimizer_relight, epoch)

        print("Validation Relight")
        loss_mean_relight = eval_relight(model_decomposition, model_relight, val_data_loader, epoch)
